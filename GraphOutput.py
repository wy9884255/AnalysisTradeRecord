...
def GenGraph(code, stock_name, buy_date, sell_date, buy_price, sell_price):
    ...
    #gen data list for display usage
    date_list = []
    for i in range((sell_date_extend - buy_date_extend).days):
        date_list.append(sell_date_extend.strftime('%Y/%m/%d'))
        sell_date_extend = sell_date_extend + day
    ...
    line = (
        Line()
            ...
            .add_yaxis(
            series_name="MA10",
            y_axis=tl.SMA(df_data['close'], 10),
            is_smooth=True,
            is_hover_animation=False,
            linestyle_opts=opts.LineStyleOpts(width=1, opacity=0.5),
            label_opts=opts.LabelOpts(is_show=False),
            ...
            .add_yaxis(
            series_name="MA20",
            y_axis=tl.SMA(df_data['close'], 20),
            is_smooth=True,
            is_hover_animation=False,
            linestyle_opts=opts.LineStyleOpts(width=1, opacity=0.5),
            label_opts=opts.LabelOpts(is_show=False),
            ...
            .add_yaxis(
            series_name="MA60",
            y_axis=tl.SMA(df_data['close'], 60),
            is_smooth=True,
            is_hover_animation=False,
            linestyle_opts=opts.LineStyleOpts(width=1, opacity=0.5),
            label_opts=opts.LabelOpts(is_show=False),
